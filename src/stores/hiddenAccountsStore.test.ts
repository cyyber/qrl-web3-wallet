...
describe("HiddenAccountsStore", () => {
  ...
  describe("loadHiddenAccounts", () => {
    it("should load hidden accounts from storage", async () => {
      localStore["HIDDEN_ACCOUNTS"] = {
        Q0000000000000000000000000000000000000000000000000000000020B714091cF2a62DADda2847803e3f1B9D2D3779: true,
      };
  ...
  describe("hideAccount", () => {
    it("should mark an account as hidden", async () => {
      await store.hideAccount("Q0000000000000000000000000000000000000000000000000000000020B714091cF2a62DADda2847803e3f1B9D2D3779");

      expect(
        store.isHidden("Q0000000000000000000000000000000000000000000000000000000020B714091cF2a62DADda2847803e3f1B9D2D3779"),
      ).toBe(true);
    });
    ...
    it("should preserve existing hidden accounts", async () => {
      await store.hideAccount("Q0000000000000000000000000000000000000000000000000000000020B714091cF2a62DADda2847803e3f1B9D2D3779");
      await store.hideAccount("Q0000000000000000000000000000000000000000000000000000000020fB08fF1f1376A14C055E9F56df80563E16722b");

      expect(
        store.isHidden("Q0000000000000000000000000000000000000000000000000000000020B714091cF2a62DADda2847803e3f1B9D2D3779"),
      ).toBe(true);
      expect(
        store.isHidden("Q0000000000000000000000000000000000000000000000000000000020fB08fF1f1376A14C055E9F56df80563E16722b"),
      ).toBe(true);
    });
  });
  ...
  describe("unhideAccount", () => {
    it("should remove an account from hidden list", async () => {
      await store.hideAccount("Q0000000000000000000000000000000000000000000000000000000020B714091cF2a62DADda2847803e3f1B9D2D3779");
      await store.unhideAccount("Q0000000000000000000000000000000000000000000000000000000020B714091cF2a62DADda2847803e3f1B9D2D3779");
